...
namespace SpaceEngineersRykeMiner
{
    public sealed class Program : MyGridProgram
    {
        ...
        public void Main(string argument)
        {
            ...
            switch (state)
            {
                ...
                case State.MINING:
                    height = 30;
                    Follower1.GoToPos(currentTargetPos);
                    LCD.WriteText($"Mining...{(float)tick / timeToMine * 100}%");
                    if (tick++ > timeToMine)
                    {
                        Idle();
                    }
        ...
        {
            var msg = $"{botName};request";
            IGC.SendBroadcastMessage("Ch2", $"{msg}", TransmissionDistance.AntennaRelay);
            Echo("GetSpot");
        }
        ...
        public class Follower
        {
            ...
            public double GoToPos(Vector3D Pos)
            {
                ...
                Pos += -Vector3D.Normalize(GravAccel) * ParentProgram.height;
            ...
            private class MyGyros
            {
                ...
                {
                    GyroOver(true);
                    Vector3D grav = Vector3D.Normalize(RemCon.GetNaturalGravity());
                    Vector3D axis = grav.Cross(RemCon.WorldMatrix.Down);
                    if (grav.Dot(RemCon.WorldMatrix.Down) < 0)
                    {
